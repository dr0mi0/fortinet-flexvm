...
start_date = get_first_date_of_current_month(datetime.now().year, datetime.now().month)
end_date = get_last_date_of_month(datetime.now().year, datetime.now().month)

print(start_date)
print(end_date)

# For each CONFIG available and for the date range selected, print the config name, ID & parameters, the VM list associated and the points consumed by VM.
for idx in range(configs.shape[0]):
    print('#' * 120 + '\n' + '#' * 120 + '\n')
    print('\nConfig\n------\n')
    print(configs.iloc[idx][conf_cols].to_markdown(tablefmt='grid'))
    print('\nConfig Parameters\n-----------------\n')
    print(pd.DataFrame(configs.iloc[idx]['parameters']).to_markdown(tablefmt='grid', index = False))
    print('\nVM list\n-------\n')
    ...
    points = vms_points_by_config_id(api_access_token, prog, start_date, end_date)
    if(points['vms'] != []):
        print(pd.DataFrame(points['vms'])[['serialNumber', 'points']].to_markdown(tablefmt='grid', index = False))
    else:
